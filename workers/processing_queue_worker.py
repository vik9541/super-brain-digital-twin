...
async def run_face_recognition(file_url: str, file_path: str) -> Dict[str, Any]:
    ...
    return {
        "faces_detected": 0,
        "faces": [],
        "note": "Face recognition not implemented yet"
    }
...
async def run_extract_table(file_url: str, file_path: str) -> Dict[str, Any]:
    ...
    return {
        "tables_found": 0,
        "tables": [],
        "note": "Table extraction not implemented yet"
    }
    ...
    try:
        ...
        await conn.execute("""
            ...
            SET status = 'done',
                ...
                duration_ms = $3
            WHERE id = $4
        """, result, datetime.now(), duration_ms, item_id)
        ...
        if retry_count >= max_retries:
            ...
            await conn.execute("""
                ...
                SET status = 'failed',
                    ...
                    completed_at = $4
                WHERE id = $5
            """, str(e), str(e.__traceback__), retry_count, datetime.now(), item_id)
            
            await send_telegram_notification(f"❌ <b>Ошибка обработки:</b> {processing_type}\n{str(e)}")
            ...
            await conn.execute("""
                ...
                SET status = 'pending',
                    ...
                    error_message = $2
                WHERE id = $3
            """, retry_count, str(e), item_id)
            
            logger.warning(f"⚠️ Processing failed, will retry ({retry_count}/{max_retries}): {item_id}")
