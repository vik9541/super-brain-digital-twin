...
async def test_03_redis_connection():
    ...
    try:
        ...
        await redis_client.setex(test_key, timedelta(hours=ttl_hours), test_value)
...
async def test_07_file_upload():
    ...
    async with httpx.AsyncClient(timeout=TIMEOUT) as client:
        ...
        files = {"file": ("test_smoke.txt", test_content, "text/plain")}
...
async def test_09_batch_processing():
    ...
    async with httpx.AsyncClient(timeout=TIMEOUT) as client:
        # Trigger batch job
        response = await client.post(f"{API_URL}/batch/trigger", json={"test": True})
    ...
    async with httpx.AsyncClient(timeout=TIMEOUT) as client:
        ...
        for endpoint in endpoints:
            ...
            try:
                ...
                results.append(
                    {"endpoint": endpoint, "status": response.status_code, "time": elapsed}
                )
        ...
        assert (
            avg_time < PERFORMANCE_THRESHOLD_API
