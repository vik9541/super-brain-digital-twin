...
from api.workspaces.models import (
    WorkspaceCreate, WorkspacePlan, WorkspaceMemberInvite, WorkspaceRole
)
...
class MockTable:
    ...
    def delete(self):
        self.data[self.table_name] = [
            item for item in self.data[self.table_name]
...
async def test_create_shared_list():
    ...
    shared_list = await service.create_shared_list(
        workspace.id,
        user_id,
        list_data
    )
