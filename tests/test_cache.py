...
async def redis_client():
    ...
    client = redis.Redis(
        host="localhost", port=6379, password="superbrain_redis_2025", decode_responses=False
    )
...
async def cache_manager(redis_client):
    ...
    manager = CacheManager(
        redis_client=redis_client, default_ttl=86400, key_prefix="test_superbrain"
    )
    yield manager
    await manager.close()
...
async def test_cache_hit(cache_manager):
    ...
    recommendations = [
        ContactRecommendation(contact_id=f"rec_{i}", score=0.9 - i * 0.05) for i in range(k)
    ]

    # Set cache
    success = await cache_manager.set_recommendations(workspace_id, contact_id, recommendations, k)
...
async def test_ttl_expiration(cache_manager):
    ...
    await cache_manager.set_recommendations(workspace_id, contact_id, recs, k=1, ttl=2)
...
async def test_cache_warmup(cache_manager):
    ...
    async def mock_generate_recommendations(ws_id: str, contact_id: str, k: int):
        return [ContactRecommendation(contact_id=f"rec_{i}", score=0.9) for i in range(k)]
...
async def test_redis_connection_failure():
    ...
    success = await cache_manager.set_recommendations(
        "ws_1", "c_1", [ContactRecommendation(contact_id="c1", score=0.8)], k=1
    )
