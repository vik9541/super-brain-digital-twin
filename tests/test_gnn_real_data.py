...
def test_contact_id(supabase_client, test_workspace_id):
    ...
    result = (
        supabase_client.table("apple_contacts")
        .select("id")
...
@pytest.mark.asyncio
async def test_recommendations_with_real_workspace(
    supabase_client, test_workspace_id, test_contact_id
):
    ...
    result = await recommender.get_recommendations(
        workspace_id=test_workspace_id, contact_id=test_contact_id, k=20, explain=True
    )
...
async def test_training_with_real_data(supabase_client, test_workspace_id):
    ...
    result = await recommender.train_model(
        workspace_id=test_workspace_id, epochs=10, learning_rate=0.01
    )
...
async def test_model_persistence(supabase_client, test_workspace_id, test_contact_id, tmp_path):
    ...
    result1 = await recommender.get_recommendations(
        workspace_id=test_workspace_id, contact_id=test_contact_id, k=10
    )
...
async def test_recommendation_quality(supabase_client, test_workspace_id, test_contact_id):
    ...
    # Get recommendations
    result = await recommender.get_recommendations(
        workspace_id=test_workspace_id, contact_id=test_contact_id, k=20
    )
    ...
    if recommendations:
        assert (
            recommendations[0]["confidence"] >= 0.8
        ), "Top recommendation should have high confidence"
...
async def test_edge_cases_real_data(supabase_client):
    ...
    try:
        await recommender.get_recommendations(
            workspace_id="invalid_workspace_id", contact_id="some_contact", k=10
        )
    ...
    if result.data:
        ...
        try:
            result = await recommender.get_recommendations(
                workspace_id=workspace_id, contact_id="invalid_contact_id", k=10
            )
