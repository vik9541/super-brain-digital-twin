...
class TestGNNTrainer:
    ...
    async def test_training_convergence(self, dummy_graph):
        ...
        # Train for 5 epochs
        history = await trainer.train(graph_data=dummy_graph, epochs=5, learning_rate=0.01)
    ...
    async def test_contrastive_loss_computation(self, dummy_graph):
        ...
        loss = trainer.compute_contrastive_loss(
            embeddings=embeddings, edge_index=dummy_graph.edge_index, negative_samples=5
        )
    ...
    async def test_predict_after_training(self, dummy_graph):
        ...
        # Predict
        predictions = await trainer.predict(graph_data=dummy_graph, target_idx=0, k=10)
