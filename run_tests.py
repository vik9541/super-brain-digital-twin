...
class TestSuite:
    ...
    def create_test_run(self, run_name: str):
        ...
        try:
            ...
            response = self.supabase.rpc(
                "fn_create_test_run",
                {"p_run_name": run_name, "p_environment": "production", "p_triggered_by": "CLI"},
            ).execute()
    ...
    def test_kubernetes_health(self) -> TestResult:
        ...
        try:
            result = subprocess.run(
                ["kubectl", "get", "pods", "-A"], capture_output=True, timeout=10
            )
    ...
    def test_dns_resolution(self) -> TestResult:
        ...
        try:
            result = subprocess.run(
                ["nslookup", "97v.ru", "8.8.8.8"], capture_output=True, timeout=5
            )
    ...
    def test_api_analysis_endpoint(self) -> TestResult:
        ...
        try:
            ...
            response = requests.get(
                f"{API_URL}/api/v1/analysis/test-id", headers=headers, timeout=5
            )
    ...
    def print_summary(self):
        ...
        avg_response_time = (
            sum(r.response_time_ms or 0 for r in self.results) / total if total > 0 else 0
        )
