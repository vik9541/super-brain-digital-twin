...
async def get_redis_client() -> redis.Redis:
    ...
    return redis.Redis.from_url(
        REDIS_URL, password=REDIS_PASSWORD, decode_responses=True, encoding="utf-8"
    )
...
async def store_file_metadata(
    user_id: int, file_id: str, file_name: str, file_type: str, file_size: int, local_path: str
) -> bool:
    """
...
async def process_image(file_path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        if not OPENAI_API_KEY:
            return {"type": "image", "status": "error", "error": "OPENAI_API_KEY not configured"}
