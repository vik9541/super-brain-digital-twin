...
class GDPRManager:
    ...
    async def export_user_data(
        ...
        try:
            ...
            # 3. Interactions
            interactions = self.supabase.table("interactions").select("*").eq("workspace_id", workspace_id).execute()
            export_data["interactions"] = interactions.data
            
            # 4. Workspaces
            workspaces = self.supabase.table("workspaces").select("*").eq("user_id", user_id).execute()
            export_data["workspaces"] = workspaces.data
            
            # 5. Notes (if any)
            notes = self.supabase.table("notes").select("*").eq("workspace_id", workspace_id).execute() if hasattr(self.supabase, 'notes') else []
...
"""
    ...
    async def get_export_status(self, export_id: str) -> Dict:
        ...
        try:
            ...
            if os.path.exists(zip_path):
                ...
                # Check operation log
                ops = self.supabase.table("gdpr_operations").select("*").eq("export_id", export_id).execute()
    ...
    async def get_data_locations(self, user_id: str) -> Dict:
        ...
        try:
            ...
            workspaces = self.supabase.table("workspaces").select("id").eq("user_id", user_id).execute()
            ...
            for ws_id in workspace_ids:
                contacts = self.supabase.table("contacts").select("id", count="exact").eq("workspace_id", ws_id).execute()
                interactions = self.supabase.table("interactions").select("id", count="exact").eq("workspace_id", ws_id).execute()
            ...
            return {
                ...
                "file_storage": {
                    "exports": f"{self.exports_dir}/",
                    "cached_models": "models/gnn/"
                },
                "third_party": {
