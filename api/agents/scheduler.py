...
async def job_generate_embeddings():
    ...
    try:
        ...
        result = service.supabase.table("contacts").select(
            "id, first_name, last_name, organization, tags, notes, updated_at"
        ).gte("updated_at", yesterday.isoformat()).execute()
...
async def job_predict_churn():
    ...
    try:
        ...
        result = predictor.supabase.table("contacts").select(
            "id, first_name, last_name, influence_score"
        ).order("influence_score", desc=True).limit(1000).execute()
...
async def job_analyze_sentiment():
    ...
    try:
        ...
        # Query all contacts
        result = analyzer.supabase.table("contacts").select(
...
async def job_generate_recommendations():
    ...
    try:
        ...
        result = engine.supabase.table("contacts").select(
            "id, first_name, last_name, influence_score"
        ).order("influence_score", desc=True).limit(1000).execute()
