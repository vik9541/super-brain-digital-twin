...
from .models import (
    WorkspaceCreate, WorkspaceResponse, WorkspaceMemberInvite,
    SharedContactListCreate, ActivityLogEntry, NotificationResponse,
    WorkspacePlan, WorkspaceRole
)
...
class WorkspaceService:
    ...
    async def create_workspace(self, user_id: UUID, workspace_data: WorkspaceCreate) -> WorkspaceResponse:
    ...
    async def get_workspace(self, workspace_id: UUID, user_id: UUID) -> WorkspaceResponse:
        ...
        try:
            ...
            # 2. Get workspace
            workspace = await self.db.table('workspaces')\
                .select('*')\
            ...
            # 3. Get members
            members = await self.db.table('workspace_members')\
                .select('*')\
                .eq('workspace_id', str(workspace_id))\
                .execute()
    ...
    async def list_user_workspaces(self, user_id: UUID, page: int = 1, per_page: int = 10):
        ...
        try:
            ...
            # Get total count
            total = await self.db.table('workspace_members')\
                .select('*', count='exact')\
                .eq('user_id', str(user_id))\
                .execute()
    ...
    async def invite_member(self, workspace_id: UUID, user_id: UUID, invite_data: WorkspaceMemberInvite) -> Dict[str, Any]:
        """Invite user to workspace"""
        try:
            # 1. Check inviter has permission (admin or owner)
            inviter = await self.db.table('workspace_members')\
                .select('role')\
                .eq('workspace_id', str(workspace_id))\
                .eq('user_id', str(user_id))\
                .execute()
            ...
            existing = await self.db.table('workspace_members')\
                .select('*')\
                .eq('workspace_id', str(workspace_id))\
    ...
    async def remove_member(self, workspace_id: UUID, user_id: UUID, member_id: UUID) -> Dict[str, Any]:
        """Remove member from workspace"""
        try:
            # 1. Check remover has permission
            remover = await self.db.table('workspace_members')\
                .select('role')\
                .eq('workspace_id', str(workspace_id))\
                .eq('user_id', str(user_id))\
                .execute()
            ...
            target = await self.db.table('workspace_members')\
                .select('role')\
                .eq('workspace_id', str(workspace_id))\
    ...
    async def create_shared_list(self, workspace_id: UUID, user_id: UUID, list_data: SharedContactListCreate):
        """Create shared contact list"""
        try:
            shared_list = await self.db.table('shared_contact_lists').insert({
                'workspace_id': str(workspace_id),
                'name': list_data.name,
    ...
    async def _log_activity(self, workspace_id: UUID, user_id: UUID, action: str, description: str = '', contact_id: Optional[UUID] = None):
    ...
    async def _create_notification(self, user_id: UUID, workspace_id: UUID, type: str, title: str, message: str, data: Dict):
        """Create notification"""
        try:
    ...
    async def get_notifications(self, user_id: UUID, workspace_id: UUID, limit: int = 20):
        ...
        try:
            notifications = await self.db.table('notifications')\
                .select('*')\
                .eq('user_id', str(user_id))\
                .eq('workspace_id', str(workspace_id))\
            ...
            unread = await self.db.table('notifications')\
                .select('*', count='exact')\
                .eq('user_id', str(user_id))\
                .eq('workspace_id', str(workspace_id))\
                .eq('read', False)\
    ...
    async def mark_notification_read(self, notification_id: UUID):
        ...
        try:
            await self.db.table('notifications').update(
                {'read': True}
