...
class ContactGraphBuilder:
    ...
    async def build_graph(
        ...
        logger.info(
            f"Graph built: {node_features.shape[0]} nodes, "
    ...
    async def _fetch_contacts(self, user_id: str) -> List[Dict]:
        ...
        try:
            response = self.supabase.table('apple_contacts')\
    ...
    def _build_contact_mapping(self, contacts: List[Dict]) -> None:
        ...
        self.contact_id_to_idx = {
            contact['id']: idx 
            for idx, contact in enumerate(contacts)
        }
        self.idx_to_contact_id = {
            idx: contact['id'] 
            for idx, contact in enumerate(contacts)
        }
    ...
    async def _build_edges_from_interactions(
        ...
        try:
            ...
            response = self.supabase.table('contact_activity_log')\
