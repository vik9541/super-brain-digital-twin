...
class ContactRecommenderGNN(torch.nn.Module):
    ...
    def __init__(
        ...
        logger.info(
            f"✅ GNN Model created: {num_layers} layers, " f"{in_features}→{hidden_dim}→{out_dim}"
        )
    ...
    def get_recommendations(
        self, embeddings: torch.Tensor, target_idx: int, k: int = 20, exclude_indices: list = None
    ) -> torch.Tensor:
        """
        ...
        similarities = F.cosine_similarity(target_embedding, embeddings_norm, dim=1)  # [num_nodes]
