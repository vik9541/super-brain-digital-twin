...
class ContactClusteringService:
    ...
    async def cluster_contacts(self, n_clusters: int = 5) -> Dict:
        ...
        try:
            ...
            if not embeddings_response.data or len(embeddings_response.data) < n_clusters:
                logger.warning(f"Insufficient embeddings for clustering: {len(embeddings_response.data) if embeddings_response.data else 0}")
                return {
                    'total_clusters': 0,
                    'clusters': {},
                    'cluster_sizes': {},
    ...
    async def infer_cluster_topics(
        self,
        cluster_id: int,
        ...
        try:
            ...
            contacts_response = self.supabase.table('contacts').select('tags, organization').in_('id', contact_ids).execute()
    ...
    async def get_cluster_members(self, cluster_id: int) -> List[Dict]:
        ...
        try:
            # Get cluster
            cluster_response = self.supabase.table('contact_clusters').select('*').eq('cluster_id', cluster_id).execute()
            ...
            # Get contact details
            contacts_response = self.supabase.table('contacts').select('*').in_('id', contact_ids).execute()
