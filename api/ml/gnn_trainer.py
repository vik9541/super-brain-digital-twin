...
class GNNTrainer:
    ...
    def create_model(self, in_features=3, hidden_dim=64, out_dim=128):
        ...
        self.model = ContactRecommenderGNN(
            in_features=in_features, hidden_dim=hidden_dim, out_dim=out_dim
        ).to(self.device)
    ...
    def compute_contrastive_loss(
        self, embeddings: torch.Tensor, edge_index: torch.Tensor, negative_samples: int = 5
    ) -> torch.Tensor:
        """
        ...
        pos_scores = F.cosine_similarity(
            embeddings[pos_src], embeddings[pos_dst], dim=1
        )  # [num_edges]
    ...
    async def train(
        ...
        for epoch in range(epochs):
            ...
            loss = self.compute_contrastive_loss(
                embeddings, edge_index, negative_samples=negative_samples
            )
