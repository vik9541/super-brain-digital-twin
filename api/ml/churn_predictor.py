...
class ChurnPredictor:
        ...
        try:
            ...
            if self.model is None:
                ...
                try:
                    ...
                    churn_prob = self.model.predict_proba(features_array)[0][
                        1
    ...
    async def train_model(self, training_data: List[Tuple[List[float], int]]) -> Dict:
        ...
        try:
            ...
            logger.info(
                f"Model trained: accuracy={accuracy:.3f}, precision={precision:.3f}, recall={recall:.3f}"
            )
    ...
    async def _extract_features(self, contact_id: str) -> Optional[List[float]]:
        ...
        try:
            # Get contact data
            contact_response = (
                self.supabase.table("contacts").select("*").eq("id", contact_id).execute()
            )
            ...
            interaction_response = (
                self.supabase.table("sync_history")
                .select("id")
                .eq("contact_id", contact_id)
                .gte("synced_at", ninety_days_ago)
                .execute()
            )

            interaction_count = len(interaction_response.data) if interaction_response.data else 0
            interaction_freq = min(
                interaction_count / 3.0, 1.0
            )  # Normalize: 3+ interactions in 90 days = 1.0
            ...
            if community_id is not None:
                community_response = (
                    self.supabase.table("communities")
                    .select("size")
                    .eq("id", community_id)
                    .execute()
                )
