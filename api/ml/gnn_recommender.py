...
class GNNRecommender:
    ...
    async def get_recommendations(
        ...
        try:
            logger.info(f"Getting GNN recommendations for contact {contact_id} in workspace {workspace_id}")
            
            # 1. Build graph
            from api.ml.graph_builder import ContactGraphBuilder
            graph_builder = ContactGraphBuilder(self.supabase)
            
            graph_data, contact_ids, id_to_idx = \
                await graph_builder.build_graph_for_workspace(workspace_id)
            ...
            top_indices = model.get_recommendations(
                embeddings,
                target_idx,
                k=k,
                exclude_indices=[]
            ...
            for rank, rec_idx in enumerate(top_indices, 1):
                ...
                # Get contact details
                contact = await graph_builder.get_contact_details(
                    workspace_id,
                    rec_contact_id
                )
                ...
                similarity = torch.nn.functional.cosine_similarity(
                    ...
                    embeddings[rec_idx_int].unsqueeze(0)
                ).item()
    ...
    async def train_model(
        ...
        try:
            ...
            final_loss = await trainer.train(
                graph_data,
                epochs=epochs
            )
