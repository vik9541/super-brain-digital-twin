...
class ErrorHandler:
    ...
    async def handle_error(self, error: Exception, context: Dict[str, Any]):
        ...
        try:
            ...
            # Log error
            logger.error(
                f"Error {error_id}: {error_data['type']} - {error_data['message']}", extra=context
            )
            ...
            if "chat_id" in context:
                await self.notify_user_error(context["chat_id"], error, error_id)
...
"""
            ...
            await self._send_telegram_message(int(ADMIN_TELEGRAM_ID), admin_message)
    ...
    def _is_critical(self, error: Exception) -> bool:
        ...
        critical_types = ["DatabaseError", "ConnectionError", "TimeoutError", "AuthenticationError"]
