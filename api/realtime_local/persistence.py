...
class WebSocketPersistence:
    ...
    async def save_message(
        self, workspace_id: str, message_type: str, user_id: str, payload: Dict
    ) -> Optional[int]:
        ...
        try:
            result = (
                self.supabase.table("websocket_messages")
                .insert(
                    {
    ...
    async def get_undelivered_messages(
        self, workspace_id: str, user_id: str, since_hours: int = 24
    ) -> List[Dict]:
    ...
    async def create_session(self, workspace_id: str, user_id: str) -> Optional[int]:
        ...
        try:
            result = (
                self.supabase.table("websocket_sessions")
                .insert(
                    {
                        "workspace_id": int(workspace_id) if workspace_id.isdigit() else 0,
                        "user_id": user_id,
    ...
    async def update_session_stats(
        self, session_id: int, messages_sent: int = 0, messages_received: int = 0
    ) -> bool:
    ...
    async def get_active_sessions(self, workspace_id: str) -> List[Dict]:
        ...
        try:
            result = (
                self.supabase.from_("active_websocket_sessions")
                .select("*")
    ...
    async def get_message_stats(self, workspace_id: str) -> Dict:
        ...
        try:
            result = (
                self.supabase.from_("websocket_message_stats")
                .select("*")
                .eq("workspace_id", int(workspace_id) if workspace_id.isdigit() else 0)
                .execute()
            )
