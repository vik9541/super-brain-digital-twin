...
class GmailSyncManager:
    ...
    def __init__(self, supabase: Client):
        ...
        self.redirect_uri = os.getenv(
            "GMAIL_OAUTH_REDIRECT_URI", "http://localhost:8001/api/gmail/oauth-callback"
        )
    ...
    async def sync_contacts_and_interactions(
        self, user_id: str, workspace_id: str, max_emails: int = 500
    ) -> Dict:
        """
        ...
        try:
            ...
            results = (
                ...
                .messages()
                .list(
                    userId="me", maxResults=max_emails, q="in:sent OR in:inbox"  # Sent and received
                )
                .execute()
            )
            ...
            for msg in messages:
                try:
                    ...
                    # Track interaction
                    direction = (
                        "sent" if "From" in headers and "me" in headers["From"] else "received"
                    )
            ...
            logger.info(
                f"✅ Gmail sync complete: {emails_processed} emails, {contacts_enriched} contacts, {interactions_tracked} interactions"
            )
    ...
    async def _enrich_contact(
        self, workspace_id: str, email: str, name: str, frequency: int, last_contact: str
    ) -> bool:
    ...
    async def enrich_contact(self, contact_id: str) -> Dict:
        ...
        try:
            ...
            interactions = (
                self.supabase.table("email_interactions")
                .select("*")
    ...
    async def disconnect(self, user_id: str) -> bool:
        ...
        try:
            self.supabase.table("gmail_sync").update(
                {"enabled": False, "access_token": None, "refresh_token": None}
            ).eq("user_id", user_id).execute()
