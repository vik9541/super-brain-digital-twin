...
class ReportsGenerator:
    ...
    async def send_email_report(self, excel_file: str) -> bool:
        ...
        try:
            ...
            msg["Subject"] = (
                f"Digital Twin Hourly Report - {datetime.utcnow().strftime('%Y-%m-%d %H:%M')}"
            )
...
"""
            ...
            encode_base64(part)
            part.add_header(
                "Content-Disposition", f"attachment; filename={excel_file.split('/')[-1]}"
            )
            msg.attach(part)
    ...
    async def send_error_alert(self, error: str):
        ...
        try:
            ...
            await self.telegram_bot.send_message(
                chat_id=chat_id, text=f"⚠️ ERROR in Reports Generator:\n{error}"
            )
        except:
            pass
