...
async def similar_contacts(
    ...
    # Call ML service
    results = await service.find_similar_contacts(contact_id=str(contact_id), top_n=limit)
...
async def recommended_contacts(
    ...
    results = await engine.recommend_contacts(
        user_id=str(user_id), limit=limit, min_score=min_score
    )
...
async def contact_clusters() -> List[Cluster]:
    ...
    for cluster_data in result.data:
        ...
        if contact_ids and len(contact_ids) > 0:
            ...
            contacts_result = supabase.table("contacts").select("*").in_("id", sample_ids).execute()
